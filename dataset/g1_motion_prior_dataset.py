...
MOTION_PRIOR_CACHE_VERSION = "motion_prior_v2"
DEFAULT_FRAMES = 150
DEFAULT_FPS = 30.0
...
def compute_contact_labels_for_encoded_motions(
    ...
    for start in tqdm(
        ...
        fk_feet = fk["feet"].detach().cpu().numpy()
        ...
        for key in ("support_contact", "near_support", "lowest_foot_heights", "ground"):
            labels[key].append(contact_payload[key])
        labels["target_fk_keypoints"].append(fk["keypoints"].detach().cpu().numpy())
        labels["target_fk_feet"].append(fk_feet)
    return {
...
def build_g1_motion_prior_cache(
    ...
    for split, payload in split_payloads.items():
        ...
        split_cache = {
            ...
            "target_fk_keypoints": contacts["target_fk_keypoints"],
            "target_fk_feet": contacts["target_fk_feet"],
    ...
    metadata = {
        ...
        "target_fk_cache": {
            "keypoints": "target_fk_keypoints",
            "feet": "target_fk_feet",
            "source": "decoded_motion_with_g1_torch_kinematics",
        },
...
class G1MotionPriorDataset(Dataset):
    def __init__(
        ...
        self.target_fk_keypoints = np.asarray(payload["target_fk_keypoints"][:count], dtype=np.float32)
        self.target_fk_feet = np.asarray(payload["target_fk_feet"][:count], dtype=np.float32)
        ...
        if self.target_fk_keypoints.shape[:2] != (len(self), DEFAULT_FRAMES):
            raise ValueError(
                "cached target_fk_keypoints must have leading shape "
                f"{(len(self), DEFAULT_FRAMES)}, got {self.target_fk_keypoints.shape[:2]}"
            )
        if self.target_fk_feet.shape[1:] != (DEFAULT_FRAMES, 2, 3):
            raise ValueError(
                f"cached target_fk_feet expected {(DEFAULT_FRAMES, 2, 3)}, "
                f"got {self.target_fk_feet.shape[1:]}"
            )
    ...
    def __getitem__(self, index):
        return {
            ...
            "target_fk_keypoints": torch.from_numpy(self.target_fk_keypoints[index]),
            "target_fk_feet": torch.from_numpy(self.target_fk_feet[index]),
