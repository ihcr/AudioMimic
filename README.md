...
This branch extends EDGE with beat-conditioned diffusion experiments, local G1 robot experiments, beat-aware inference, and benchmark helpers. It keeps generated data, checkpoints, renders, caches, and logs out of Git so the repository can be pushed and shared as source code.

For the current Isambard SSH / Slurm migration workflow, start with
`AGENTS.md`, `docs/NEW_SERVER_SETUP.md`, and `docs/experiments/INDEX.md`.
The branch is intended to clone directly as a normal repo and should not depend
on the old shared EDGE checkout for code or environment.
## Requirements
...
### Environment
This repo is intended to run from the repo-local `.venv311` environment. Prefer
calling the interpreter directly so commands also work when an activation script
is not present:
```.bash
...
```.bash
.venv311/bin/python -m pip install -r requirements-eval.txt
```
...
```
Native G1 rendering defaults to MuJoCo with `MUJOCO_GL=egl`.
The Matplotlib stick-figure renderer remains available only as the explicit
debug backend.

On Isambard, launch long GPU work through Slurm and keep logs under `slurm/`
and `setup_logs/`. The direct-attached 4090 workflow uses `tmux` plus live
`tee` logs only on machines that do not have Slurm.
...
does not mix with the full AIST retarget. Copy or symlink the retarget source on
the current server:
```.bash
/path/to/aist-g1-retargeted
```
Prepare the FK-beat tree from the repo root with:
```.bash
