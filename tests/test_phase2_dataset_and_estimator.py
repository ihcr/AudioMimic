...
class DatasetBeatSchemaTests(unittest.TestCase):
    ...
    def test_structured_motion_controls_are_loaded_from_memmap_store(self):
        dataset_cls = self.dataset_module.AISTPPDataset
        with TemporaryDirectory() as tmpdir:
            tmp_path = Path(tmpdir)
            ...
            np.save(wav2clip_path, np.ones((150, 706), dtype=np.float32))
            np.save(gaussian_path, np.full((150, 1), 0.5, dtype=np.float32))
            np.savez(
                motion_control_path,
                motion_intensity_envelope=np.full((150, 1), 0.25, dtype=np.float32),
                motion_beatness_envelope=np.full((150, 1), 0.75, dtype=np.float32),
            )
            fake_data = {
                "pos": np.zeros((1, 150, 3), dtype=np.float32),
                "q": np.zeros((1, 150, 72), dtype=np.float32),
            ...
            self.assertEqual(tuple(cond["control"]["motion_intensity"].shape), (150, 1))
            self.assertEqual(tuple(cond["control"]["motion_beatness"].shape), (150, 1))
            self.assertAlmostEqual(float(cond["control"]["motion_intensity"].mean()), 0.25)
            self.assertAlmostEqual(float(cond["control"]["motion_beatness"].mean()), 0.75)
            self.assertNotIn(str(motion_control_path), [path for path, _ in load_calls])
            self.assertIn((str(dataset.motion_control_store_path), "r"), load_calls)

    def test_beat_features_8d_motion_beatness_loads_beat_features_and_beatness_only(self):
        dataset_cls = self.dataset_module.AISTPPDataset
        with TemporaryDirectory() as tmpdir:
            tmp_path = Path(tmpdir)
            beat_features_path = tmp_path / "clip.npy"
            motion_control_path = tmp_path / "clip_control.npz"
            np.save(beat_features_path, np.ones((150, 8), dtype=np.float32))
            np.savez(
                motion_control_path,
                motion_intensity_envelope=np.full((150, 1), 0.25, dtype=np.float32),
                motion_beatness_envelope=np.full((150, 1), 0.75, dtype=np.float32),
            ...
            fake_data = {
                "pos": np.zeros((1, 150, 3), dtype=np.float32),
                "q": np.zeros((1, 150, 72), dtype=np.float32),
                "filenames": [str(beat_features_path)],
                "wavs": ["clip.wav"],
                "structured_condition_paths": [
                    {
                        "beat_features_8d": str(beat_features_path),
                        "motion_control": str(motion_control_path),
                    }
                ],
            }
            ...
            self.assertEqual(tuple(cond["semantic"]["beat_features_8d"].shape), (150, 8))
            self.assertEqual(tuple(cond["control"]["motion_beatness"].shape), (150, 1))
            self.assertNotIn("motion_intensity", cond["control"])
            self.assertAlmostEqual(float(cond["control"]["motion_beatness"].mean()), 0.75)

    def test_getitem_loads_feature_when_instance_attribute_shadows_loader_method(self):
        dataset_cls = self.dataset_module.AISTPPDataset
        with TemporaryDirectory() as tmpdir:
            tmp_path = Path(tmpdir)
            feature = np.full((4, 3), 4.0, dtype=np.float32)
            feature_path = tmp_path / "clip.npy"
            ...
            dataset.__dict__["_load_feature"] = dataset
            _, cond, _, _ = dataset[0]

            self.assertTrue(torch.equal(cond, torch.from_numpy(feature)))
