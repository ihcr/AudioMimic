...
def consumer_thread(pose_queue, port, topic, fps=30.0):
    """ZMQ streaming thread."""
    ctx = zmq.Context()
    sock = ctx.socket(zmq.PUB)
    sock.bind(f"tcp://*:{port}")
    print(f"[Consumer] ZMQ PUB bound on tcp://*:{port}, topic='{topic}'")
    print(f"[Consumer] Waiting for first frame...")
    ...
    while True:
        ...
        if frame_data is None:  # EOF signal
            break
...
def producer_thread(opt, pose_queue):
    ...
    for i, slice_file in enumerate(file_list):
        ...
        # 2. Extract Jukebox features for this chunk
...
if __name__ == "__main__":
    ...
    consumer = threading.Thread(
        ...
        args=(pose_queue, custom_args.port, custom_args.topic),
        daemon=True
    )
    consumer.start()

    # 5. Start Producer Thread (Diffusion Generation) in main thread
    producer_thread(opt, pose_queue)
