...
| 2026-05-26 | root-turn qualitative failure audit | analyzed, overlap conclusion superseded | audit `eval/EXP-20260524-finedance-g1-wav2clip-intensity-beatness/root_turn_audit_20260526.json`; patched metric fields in `eval/g1_metrics.py`; deleted superseded no-render final-hard-overlap probe and log; current test `tests.test_phase4_to_6_beat_integration.InferenceBeatUtilityTests.test_long_ddim_sample_does_not_hard_overwrite_final_overlap` | User-observed sudden turns are real root rotation spikes in saved G1 motion. On the same 90s song/window, GT stitched root angular max is `5.86 rad/s`, Librosa35 is `7.18`, r05 is `63.19`, and v3 is `87.41`; v3 has `99` frames above `2pi rad/s` and `42` frames above `3pi rad/s`. Full 5s eval slices show the same tail: v3/r05 root angular max reaches about `94 rad/s`, while GT max is about `18 rad/s`. The later final hard-overlap overwrite in `long_ddim_sample` was found to create 75-frame stitch gaps and has been removed; keep the original EDGE-style final stitch/blend behavior. Primary diagnosis remains: the world-frame FK keypoint speed used for motion intensity/beatness lets the model satisfy amplitude/rhythm controls by spinning root yaw, moving wrists/ankles/torso through world space even when local dance motion is not healthy. |
| 2026-05-26 | training full-eval hook | implemented | source `EDGE.py`; parser `args.py`; tests `tests/test_phase0_cli_and_preprocess.py` | Future G1 training defaults to `--full_eval_interval 500` unless explicitly disabled with `0`. At every due global epoch, training saves a checkpoint, runs the standard full eval variants, writes metrics/audit/report under `eval/<run>/ckpt<epoch>_<variant>/`, and logs numeric metrics to W&B. This is for future clean v3b/v4 runs; it does not make the current v3 checkpoint safe to resume as the mainline. |
...
The 90s render exposed a second, more urgent issue: the anti-average motion controls can be exploited through root yaw. Because `motion_intensity` and `motion_beatness` are based on world-frame FK keypoint speeds, fast root heading changes move wrists/ankles/torso in world space and can satisfy the control/loss without producing natural local body motion. This explains why both r05 and v3 have larger amplitude but sudden turns, while Librosa35 is calmer. Future acceptance must include root angular velocity metrics; v3/v3b should either make the control signal root-heading-invariant/local-frame or add an explicit root angular velocity/acceleration guard.

## Next Action

Do not resume v3 as-is until the root-turn issue is addressed. Start a focused v3b ablation before more long training: rebuild motion-control features in a root-heading-invariant/local frame, add root angular velocity metrics to every full eval, and add a light decoded root angular velocity/acceleration guard or target-matching loss. Do not reintroduce the final hard-overlap overwrite; rely on the original stitch/blend path and validate boundary jumps on long renders. Use the current v3 checkpoint only for diagnosis/renders, not as the next main training branch.
